---
name: goldy-source-types
description: Add a new source type (material kind) to Goldy with a custom hand-drawn SVG icon and colour, so references of that type render with their own badge. Use when the user wants a new kind of resource in reports (for example "video", "course", "paper", "internal wiki"), asks to add a source type, or asks for a custom icon for a material kind.
---

# Goldy Source Types: new material kinds with custom SVG icons

Goldy tags every reference with a *kind* (reference, docs, tutorial, how-to,
language, spec, book), each shown with its own SVG icon and colour. This skill
adds new kinds. Goldy uses only SVG icons, never emoji, so a new type always
...
text, `bg` for the badge background) that fits Goldy's palette.
...
```bash
python3 "$SKILL_DIR/scripts/source_types.py" add \
  --kind video --label "Video" --icon-name video \
  --color "#7a2e8a" --bg "#f3e8f7" \
  --svg '<circle cx="8" cy="8" r="6"/><path d="M6.6 5.5l4 2.5-4 2.5z"/>'
```

This writes the type to `~/.claude/goldy/source_types.json` and the icon to
`~/.claude/goldy/icons.json`. The renderer merges both at render time, so any
