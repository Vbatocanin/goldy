...
Usage:
    python3 render.py nodes.json -o goldy-report.html
"""
import argparse
import html
import json
...
def _load_py_members():
    """Merge fetched-doc tooltips Goldy has persisted for specific functions.
    Each entry is `"module.attr": "what it does"` or
    `"module.attr": {"tip": "...", "url": "..."}`."""
    p = os.path.expanduser("~/.claude/goldy/py_members.json")
    if os.path.isfile(p):
        try:
...
# goldy-source-types skill are merged from ~/.claude/goldy/source_types.json,
...
def _load_source_types():
    p = os.path.expanduser("~/.claude/goldy/source_types.json")
...
.node::before{content:"";position:absolute;left:-23px;top:28px;width:2px;
  height:calc(100% + 18px);background:var(--gold);z-index:0}
.node:last-child::before{display:none}
.k-action::before{background:var(--blue)} .k-prompt::before{background:var(--violet)}
.k-principle::before{background:var(--teal)} .k-security::before{background:#c0392b}
...
.node.collapsed .spine-dot{transform:scale(1.08)}
.node.collapsed:hover .spine-dot{transform:scale(1.2)}
...
<footer>Generated by <b>Goldy</b>, keeping you in touch with your codebase.</footer>
