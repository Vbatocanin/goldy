...
Usage:
    python3 render_index.py .goldy/reports -o .goldy/goldy-report.html
"""
import argparse
import html
import os
...
def card(entry, href):
    ...
    return (f"<a class='rcard' href='{html.escape(href)}'>"
            f"<span class='rc-mark'>{icon('goldy')}</span>"
...
.hero .goldy-mark{flex:none;width:54px;height:54px;display:grid;place-items:center;
  color:var(--gold);background:var(--gold-soft);border-radius:50%;
  box-shadow:0 2px 8px rgba(202,162,74,.25)}
.hero .goldy-mark .ic{width:32px;height:32px}
...
<body><div class="wrap">
<div class="hero"><span class="goldy-mark">{mark}</span>
  <div><span class="eyebrow">Goldy history</span>
...
def render_index(reports_dir, out_dir):
    ...
    body = "".join(cards) or "<div class='empty'>No reports yet. Generate one with the goldy skill.</div>"
    return TEMPLATE.format(
        title=esc((project or "Goldy") + " · Goldy history"),
        css=CSS, mark=icon("goldy"), project=esc(project or "This repo"),
...
def main():
    ...
    print(f"goldy: master index of {n} report(s) -> {args.out}", file=sys.stderr)
