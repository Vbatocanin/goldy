#!/usr/bin/env python3
"""Goldie project profile.

`goldie init` builds a small profile of the current repo so later Goldie reports
...
stored per project at `.goldie/profile.json`.
...
PROFILE = os.path.join(".goldie", "profile.json")
...
SKIP_DIRS = {".git", "node_modules", ".venv", "venv", "__pycache__", "dist",
             "build", ".next", "target", "vendor", ".goldie", ".idea", ".vscode"}
...
def cmd_save(args):
    ...
    os.makedirs(".goldie", exist_ok=True)
    with open(PROFILE, "w") as fh:
        json.dump(prof, fh, indent=2)
    print(f"goldie: saved profile -> {PROFILE}", file=sys.stderr)
