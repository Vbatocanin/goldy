...
Custom icons (for new source types, see the `goldie-source-types` skill) live in
`~/.goldie/icons.json` as `{ "name": "<svg inner markup>" }` and are merged
...
ICONS = {
    ...
    # (Goldie's brand mark, "goldie", is a full multi-colour SVG, not a currentColor
    # glyph. It lives in goldie_mark.svg and is special-cased in icon() below.)
    # tools
...
def _ensure_loaded():
    ...
    if _loaded:
        return
    _loaded = True
    p = os.path.expanduser("~/.goldie/icons.json")
    if os.path.isfile(p):
        try:
...
def _goldie_inner():
    """The inner markup of the Goldie brand mark (a full multi-colour golden wave),
    loaded once from goldie_mark.svg beside this file and stripped of its <svg>
    wrapper so icon() can re-wrap it at the requested size."""
    global _goldie_mark
    if _goldie_mark is None:
        p = os.path.join(os.path.dirname(os.path.abspath(__file__)), "goldie_mark.svg")
        try:
            m = re.search(r"<svg\b[^>]*>(.*)</svg>", open(p).read(), re.S)
            _goldie_mark = m.group(1).strip() if m else ""
        except OSError:
            _goldie_mark = ""
    return _goldie_mark
