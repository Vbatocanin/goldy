#!/usr/bin/env python3
"""Goldie resource library.

A small, persistent index of learning resources (URLs the user trusts, crawled
and distilled into "what it can teach") so Goldie can prefer them when enriching
a report. The library is cross-project, stored at:

    ~/.goldie/resources.json   (override with $GOLDIE_RESOURCES)
...
Usage:
    ...
    resources.py materials "<terms>" [-n 3]   # ready-to-merge Goldie material entries
    resources.py path
"""
import argparse
import datetime
...
def lib_path():
    return os.environ.get("GOLDIE_RESOURCES",
                          os.path.expanduser("~/.goldie/resources.json"))
...
def cmd_materials(args):
    """Emit Goldie material entries (title/url/note/kind/summary) for a query,
